...
void transmitLEDState(void) {
  ...
  snprintf(ledStatusJSON, 20, {LED%d}rn, ledStatus);
...
int main(void) {
  ...
  while (1) {
    ...
    snprintf(data_buffer, sizeof(data_buffer),
             {Temperature%.4f,LDR%d}rn, celsius, LDRValue);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef huart) {
  ...
  if (UART1_rxBuffer == 'r') {  when enter is pressed go to this condition
    data_buffer[count++] = '0';
    char sentBuff[60];
    sprintf(sentBuff, %srn, data_buffer);
    ...
    if (strncmp(data_buffer, RGB_, 4) == 0) {
      ...
      sprintf(sentBuff, RED%d, GREEN%d,BLUE%d rn, RED, GREEN, BLUE);
      HAL_UART_Transmit(&huart1, (uint8_t )sentBuff, strlen(sentBuff), 100);
    }
